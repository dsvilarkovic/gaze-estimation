...
class ImageUnit(nn.Module):
    def __init__(self, image_height, image_width):
        ...
        self.fc_bn = nn.BatchNorm1d(image_unit_out)
...
#         self.landmarkUnit.load_state_dict(torch.load('best_136_model.py'))
        ...
        self.fc1_bn = nn.BatchNorm1d((image_unit_out + int(feature_size/4))/ 4)
        self.relu1 = nn.ReLU()

        self.fc2 = nn.Linear(in_features=(image_unit_out + int(feature_size/4))/ 4,
                             out_features=(image_unit_out + int(feature_size/4))/ 16)
        self.fc2_bn = nn.BatchNorm1d((image_unit_out + int(feature_size/4))/ 16)
        self.relu2 = nn.ReLU()
    ...
    def forward(self, x):
        landmarks = x[:, 0:136]
        #SLIKA
        image = x[:, 136:].view(-1, image_height, image_width, 3) 
